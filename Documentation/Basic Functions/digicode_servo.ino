...
void loop(){
  ...
  if (keypad_matrix){ // Si une touche est appuyée
    ...
    if (compteur == 5){ // Si on a 4 valeurs, on vérifie si le code est bon sinon on remet le compteur à 0
      if (code[0] == secretCode[0] && code[1] == secretCode[1] && code[2] == secretCode[2] && code[3] ==  secretCode[3] && code[4] =='D'){ // Si le code est bon
      ...
      if (code[0] != secretCode[0] || code[1] != secretCode[1] || code[2] != secretCode[2] || code[3] !=secretCode[3] && code[4] =='D'){ // Si le code n'est pas bon
